...
function CameraShaker:Update(deltaTime : number)
	...
	for i = 1,#instances do
		if not instances[i] then continue end
		local c = instances[i]
		...
		elseif (state ~= CameraShakeState.Inactive) then
			local shake = c:UpdateShake(deltaTime)
			deltaTime *= 60
			posAddShake = posAddShake + (shake * c.PositionInfluence)
			rotAddShake = rotAddShake + (shake * deltaTime * c.RotationInfluence)
		end
...
function CameraShaker:ShakeInstanceOnce(shakeName)
	local shakeInstance
	if typeof(shakeName) == "string" then
		shakeInstance = CameraShaker.Presets[shakeName]
	elseif typeof(shakeName) == "table" then
		shakeInstance = shakeName
	end
	if typeof(shakeInstance) ~= "table" then return end
	--print(self)
	self._camShakeInstances[#self._camShakeInstances + 1] = shakeInstance
	return shakeInstance
end
